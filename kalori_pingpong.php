...
$MET = [
    'santai'      => ['label' => 'Rekreasi / santai',         'met' => 3.5],
    'doubles'     => ['label' => 'Sosial / ganda',            'met' => 4.0],
    'singles'     => ['label' => 'Reguler / tunggal',         'met' => 5.0],
    'kompetitif'  => ['label' => 'Kompetitif / pertandingan', 'met' => 7.0],
];
...
if ($_SERVER['REQUEST_METHOD']==='POST' && $u) {
    ...
    if ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        try {
            db_exec("DELETE FROM kalori_log WHERE id=$1 AND user_id=$2 AND jenis='pingpong'",
                [$id, (int)$u['id']]);
            $flash = ['type'=>'success','msg'=>'Riwayat terhapus.'];
        } catch (Throwable $e) {
            $flash = ['type'=>'danger','msg'=>'Gagal menghapus.'];
        }
        header('Location: kalori_pingpong.php'); exit;
    }
    ...
    if ($action === 'update') {
        ...
        try {
            db_exec("UPDATE kalori_log
                     ...
                     WHERE id=$6 AND user_id=$7 AND jenis='pingpong'",
...
if ($u && isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    try {
        $editing = db_one("SELECT * FROM kalori_log WHERE id=$1 AND user_id=$2 AND jenis='pingpong'",
            [$id, (int)$u['id']]);
    } catch (Throwable $e) {}
}
...
if ($u) {
    try {
        $total = (int) db_val("SELECT COUNT(*) FROM kalori_log WHERE user_id=$1 AND jenis='pingpong'", [(int)$u['id']]);
        $riwayat = db_all("SELECT id, intensitas, berat_kg, menit, met, kalori, dibuat_pada
                           FROM kalori_log
                           WHERE user_id=$1 AND jenis='pingpong'
...
require_once __DIR__.'/includes/htmx.php'; htmx_layout_start($pageTitle ?? 'Kalori Pingpong');
?>
<h2 class="mb-3"><i class="bi bi-circle-fill text-warning"></i> Kalkulator Kalori Ping Pong</h2>
<p class="text-muted small mb-3">Hitung &amp; kelola perkiraan kalori terbakar saat bermain tenis meja (ping pong). Rumus mengacu nilai <strong>MET</strong> (Compendium of Physical Activities).</p>
  ...
  <div class="col-lg-5">
      ...
      <div class="card-body">
        <form method="post" class="row g-2">
          ...
          <div class="col-12 mt-2 d-flex gap-2">
            ...
            <?php if($editing): ?>
              <a href="kalori_pingpong.php" class="btn btn-outline-secondary">Batal</a>
            <?php endif; ?>
          </div>
          <?php if(!$u): ?>
    ...
    <div class="card shadow-sm mt-3"><div class="card-body">
      ...
      <ul class="small text-muted mb-0 ps-3">
        <li><strong>MET 3.5</strong> — rekreasi santai (rally pelan)</li>
        <li><strong>MET 4.0</strong> — ganda / sosial</li>
        <li><strong>MET 5.0</strong> — tunggal reguler</li>
        <li><strong>MET 7.0</strong> — kompetitif / pertandingan intens</li>
      </ul>
    </div></div>
  </div>
