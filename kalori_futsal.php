...
$MET = [
    'santai'      => ['label' => 'Latihan ringan / fun',       'met' => 6.0],
    'doubles'     => ['label' => 'Reguler / kasual',           'met' => 8.0],
    'singles'     => ['label' => 'Intens / sparring',          'met' => 9.0],
    'kompetitif'  => ['label' => 'Kompetitif / pertandingan',  'met' => 10.0],
];
...
if ($_SERVER['REQUEST_METHOD']==='POST' && $u) {
    ...
    if ($action === 'delete') {
        $id = (int)($_POST['id'] ?? 0);
        try {
            db_exec("DELETE FROM kalori_log WHERE id=$1 AND user_id=$2 AND jenis='futsal'",
                [$id, (int)$u['id']]);
            $flash = ['type'=>'success','msg'=>'Riwayat terhapus.'];
        } catch (Throwable $e) {
            $flash = ['type'=>'danger','msg'=>'Gagal menghapus.'];
        }
        header('Location: kalori_futsal.php'); exit;
    }
    ...
    if ($action === 'update') {
        ...
        try {
            db_exec("UPDATE kalori_log
                     ...
                     WHERE id=$6 AND user_id=$7 AND jenis='futsal'",
...
if ($u && isset($_GET['edit'])) {
    $id = (int)$_GET['edit'];
    try {
        $editing = db_one("SELECT * FROM kalori_log WHERE id=$1 AND user_id=$2 AND jenis='futsal'",
            [$id, (int)$u['id']]);
    } catch (Throwable $e) {}
}
...
if ($u) {
    try {
        $total = (int) db_val("SELECT COUNT(*) FROM kalori_log WHERE user_id=$1 AND jenis='futsal'", [(int)$u['id']]);
        $riwayat = db_all("SELECT id, intensitas, berat_kg, menit, met, kalori, dibuat_pada
                           FROM kalori_log
                           WHERE user_id=$1 AND jenis='futsal'
...
require_once __DIR__.'/includes/htmx.php'; htmx_layout_start($pageTitle ?? 'Kalori Futsal');
?>
<h2 class="mb-3"><i class="bi bi-dribbble text-success"></i> Kalkulator Kalori Futsal</h2>
<p class="text-muted small mb-3">Hitung &amp; kelola perkiraan kalori terbakar saat bermain futsal. Rumus mengacu nilai <strong>MET</strong> (Compendium of Physical Activities).</p>
  ...
  <div class="col-lg-5">
      ...
      <div class="card-body">
        <form method="post" class="row g-2">
          ...
          <div class="col-12 mt-2 d-flex gap-2">
            ...
            <?php if($editing): ?>
              <a href="kalori_futsal.php" class="btn btn-outline-secondary">Batal</a>
            <?php endif; ?>
          </div>
          <?php if(!$u): ?>
    ...
    <div class="card shadow-sm mt-3"><div class="card-body">
      ...
      <ul class="small text-muted mb-0 ps-3">
        <li><strong>MET 6.0</strong> — latihan ringan / fun match</li>
        <li><strong>MET 8.0</strong> — reguler / kasual</li>
        <li><strong>MET 9.0</strong> — intens / sparring</li>
        <li><strong>MET 10.0</strong> — kompetitif / pertandingan</li>
      </ul>
    </div></div>
  </div>
