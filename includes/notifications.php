<?php
// FCM + in-app notifications helper
require_once __DIR__ . '/auth.php';

/**
 * Simpan notifikasi in-app + (opsional) kirim FCM push.
 * Konfigurasi FCM via ENV: FCM_SERVER_KEY (legacy) atau FCM_V1 (kosongkan jika belum siap).
 */
...
function push_fcm_to_user(int $userId, string $title, string $body, string $url=''): void {
    ...
    if (!$key) return; // belum dikonfigurasi -> in-app only
    try {
