...
$pa_aktivitas_img = $pa_aktivitas_img ?? '';
// Revisi (29 Juni 2026) — Nomor WA Pemandu Olahraga (KawanKeringat).
$pa_pemandu_wa = $pa_pemandu_wa ?? '6281386369207';
...
<div class="container py-3">
  ...
  <div class="card shadow-sm border-<?= $pa_warna ?> mb-3">
    ...
    <div class="card-body">
      ...
      <?php if ($pa_aktivitas_img): ?>
        <!-- Revisi (29 Juni 2026) — Gambar visual aktivitas (generated by Lovable). -->
        <div class="text-center mb-3">
          <img src="<?= htmlspecialchars($pa_aktivitas_img) ?>"
               alt="Ilustrasi Aktivitas yang Disarankan untuk <?= htmlspecialchars($pa_judul) ?>"
               class="img-fluid rounded shadow-sm border"
               style="max-height:340px;object-fit:cover" loading="lazy">
        </div>
      <?php endif; ?>
      <ul class="mb-3">
      ...
      <div class="d-flex gap-2 flex-wrap mt-3">
        <a class="btn btn-<?= $pa_warna ?> btn-sm" href="<?= htmlspecialchars($waLink) ?>" target="_blank" rel="noopener">
          <i class="bi bi-whatsapp"></i> Tanyakan les/pendampingan ini selengkapnya
        </a>
        <?php
          // Revisi (29 Juni 2026) — Tombol Pesan/Pemandu Olahraga (WA 081386369207).
          $pemanduTeks = 'Halo KawanKeringat, saya ingin memesan Pemandu Olahraga untuk '.($pa_judul ?? 'Paket Olahraga').'.';
          $pemanduLink = 'https://wa.me/'.$pa_pemandu_wa.'?text='.rawurlencode($pemanduTeks);
        ?>
        <a class="btn btn-success btn-sm" href="<?= htmlspecialchars($pemanduLink) ?>" target="_blank" rel="noopener">
          <i class="bi bi-person-badge-fill"></i> Pesan / Pemandu Olahraga
        </a>
